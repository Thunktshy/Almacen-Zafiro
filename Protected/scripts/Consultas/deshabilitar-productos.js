...
import { getAllProducts } from "/admin-resources/scripts/Database/productManager.js";
...
import { disableProduct } from "/admin-resources/scripts/Database/productManager.js";
...
async function loadProducts() {
  try {
    ...
    if (Array.isArray(products) && products.length > 0) {
      ...
      $('#productsTable').on('click', '.select-btn', async function() {
        const id = $(this).data('id');
        console.log("Producto seleccionado: " + id);
      
        try {
          // Call the disable function with the product id
          const result = await disableProduct(id);
          alert(result.message);
      
          // Optionally refresh the product list
          // For example, if loadProducts refreshes the table:
          loadProducts();
        } catch (error) {
